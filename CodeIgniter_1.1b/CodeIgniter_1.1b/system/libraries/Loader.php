...
class _Loader {
	...
	*/
	function _Loader()
	{
		/*
		|
		| This allows the $this->load function to work from
		| within controller constructors. 
	...
	*/	
	function database($db = '', $return = FALSE)
	{
		if ($return === TRUE)
		{
			return $this->init_database($db, TRUE);
		}
		else
		{
			$this->init_database($db);
		}
	}
	...
	*/
	function script($scripts = array())
	{
		...
		foreach ($scripts as $script)
		{
			...
			if ( ! include_once(APPPATH.'scripts/'.$script.EXT))
			{
	...
	*/
	...
	{	
		...
		/*
		|----------------------------------------------
		| Reassign objects if PHP 4
		|----------------------------------------------
		|
		| We have a bit of an issue due to the way PHP 4
		| protects the scope of objects so we need to
		| reassign the $CI object if PHP 4 is being used.
		|
		*/
		if (phpversion() < '5')
		{
