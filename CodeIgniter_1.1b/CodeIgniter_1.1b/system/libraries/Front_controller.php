...
define('APPVER', '1.1 Beta');

if ( ! defined('E_STRICT')) // Some PHP versions (4.3.1) don't support this 
{
...
| the class object.  We use it internally in various classes.
| Example:
|
...
function call($class, $function, $params = array())
{
	...
	if ($class == 'CI')
	{
		...
		return call_user_func_array(array($OBJ, $function), array($params));		
	}
	else
	{
		if ( ! in_array($class, $OBJ->ci_is_loaded))
		{ 
			log_message('error', "Attempting to access a class that does not exist: ".$class);
			return FALSE;
		}
		
		if ( ! method_exists($OBJ->$class, $function))
		{ 
			log_message('error', "Attempting to call a function that does not exist: ".$function);
			return FALSE;
		}

		return call_user_func_array(array($OBJ->$class, $function), array($params));
	}
}
